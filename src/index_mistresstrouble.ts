...
async function run() {
	initMetrics(8014, "mistresstrouble");

	const acc = accounts.mistresstrouble;
	...
	conn.Player.SetDescription(MistressTroubleGameRoom.description);

	testLogic = new MistressTroubleGameRoom(conn);
